...
export default function EditVisitorPreapproval({
    preapproval,
    apartments,
}: {
    preapproval: { id: number; qr_code: string } & PreapprovalData;
    apartments: Apartment[];
}) {
    const { data, setData, put, processing, errors } = useForm<PreapprovalData>({
        visitor_name: preapproval.visitor_name,
        visitor_phone: preapproval.visitor_phone ?? '',
        apartment_id: preapproval.apartment_id ? String(preapproval.apartment_id) : '',
        expected_arrival: preapproval.expected_arrival ?? '',
        expires_at: preapproval.expires_at ?? '',
        purpose: preapproval.purpose ?? '',
        status: preapproval.status,
        entry_type: preapproval.entry_type ?? 'visitor',
    });

    return (
        <>
            <Head title="Edit Visitor Preapproval" />

            <div className="mx-auto flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <Heading
                    variant="small"
                    title="Edit Visitor Preapproval"
                    description="Update preapproval details"
                />

                <div className="rounded-lg border bg-muted/50 p-4 text-sm">
                    <p className="font-medium">QR Code</p>
                    <p className="mt-1 font-mono text-xs text-muted-foreground">{preapproval.qr_code}</p>
                </div>

                <form
                    onSubmit={(e) => {
                        e.preventDefault();
                        put(`/visitor-preapprovals/${preapproval.id}`);
                    }}
                    className="space-y-6"
                >
                    ...
                    <div className="flex items-center gap-4">
                        <Button disabled={processing}>Update Preapproval</Button>
                    </div>
                </form>
            </div>
        </>
