...
import { update } from '@/routes/commercial-tenants';
...
export default function EditCommercialTenant({
    tenant,
    buildings,
    users,
}: {
    ...
    const { data, setData, put, processing, errors } = useForm<TenantData>({
        building_id: tenant.building_id ? String(tenant.building_id) : '',
        user_id: tenant.user_id ? String(tenant.user_id) : '',
        company_name: tenant.company_name ?? '',
        contact_name: tenant.contact_name ?? '',
        email: tenant.email ?? '',
        phone: tenant.phone ?? '',
        unit_number: tenant.unit_number,
        unit_area: tenant.unit_area ?? '0',
    ...
    return (
        <>
            <Head title="Edit Commercial Tenant" />
            <div className="mx-auto flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <Heading variant="small" title="Edit Commercial Tenant" description="Update the commercial tenant details" />
                <form onSubmit={save} className="space-y-6">
                    ...
                    <div className="flex items-center gap-4">
                        <Button disabled={processing}>Update tenant</Button>
                    </div>
                </form>
            </div>
        </>
