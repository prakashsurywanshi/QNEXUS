...
export default function CreateBudget({
    ...
    const { data, setData, post, processing, errors } = useForm<BudgetData>({
        fiscal_year: '',
        account_id: '',
        budgeted_amount: '',
        actual_amount: '0',
    ...
    return (
        <>
            ...
            <div className="mx-auto flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <Heading variant="small" title="Add Budget" description="Create a budget for a chart of account" />
                <form onSubmit={save} className="space-y-6">
                    <div className="grid gap-2">
                        <Label htmlFor="fiscal_year">Fiscal year</Label>
                        <Input id="fiscal_year" name="fiscal_year" value={data.fiscal_year} onChange={(e) => setData('fiscal_year', e.target.value)} required placeholder="e.g. 2026-27" />
                    ...
                    <div className="flex items-center gap-4">
                        <Button disabled={processing}>Save budget</Button>
                    </div>
                </form>
            </div>
        </>
