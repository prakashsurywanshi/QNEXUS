...
import { update } from '@/routes/commercial-units';
...
interface UnitProps {
    ...
    commercial_tenant_id: number | null;
    unit_number: string;
    floor: string | null;
...
export default function EditCommercialUnit({
    unit,
    buildings,
    commercialTenants,
}: {
    ...
    const { data, setData, put, processing, errors } = useForm<UnitData>({
        building_id: unit.building_id ? String(unit.building_id) : '',
        commercial_tenant_id: unit.commercial_tenant_id ? String(unit.commercial_tenant_id) : '',
    ...
    return (
        <>
            <Head title="Edit Commercial Unit" />
            <div className="mx-auto flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <Heading variant="small" title="Edit Commercial Unit" description="Update the commercial unit details" />
                <form onSubmit={save} className="space-y-6">
                    ...
                    <div className="flex items-center gap-4">
                        <Button disabled={processing}>Update unit</Button>
                    </div>
                </form>
            </div>
        </>
