...
import { update } from '@/routes/lease-agreements';
...
export default function EditLeaseAgreement({
    lease,
    commercialTenants,
    users,
}: {
    ...
    const { data, setData, put, processing, errors } = useForm<LeaseData>({
        commercial_tenant_id: String(lease.commercial_tenant_id),
        user_id: lease.user_id ? String(lease.user_id) : '',
        ...
        cam_charges: lease.cam_charges ?? '0',
        rent_escalation_type: lease.rent_escalation_type,
        escalation_value: lease.escalation_value ?? '0',
        escalation_frequency_months: String(lease.escalation_frequency_months ?? 12),
        status: lease.status,
        notes: lease.notes ?? '',
    });
    ...
    const save = (e: React.FormEvent) => {
        e.preventDefault();
        put(update(lease.id).url);
    };

    return (
        <>
            <Head title="Edit Lease Agreement" />
            <div className="mx-auto flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <Heading variant="small" title="Edit Lease Agreement" description="Update the lease agreement details" />
                <form onSubmit={save} className="space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="grid gap-2">
                            ...
                            <Select value={data.commercial_tenant_id} onValueChange={(v) => setData('commercial_tenant_id', v)}>
                                <SelectTrigger id="commercial_tenant_id" className="w-full"><SelectValue /></SelectTrigger>
                                <SelectContent>
                    ...
                    <div className="flex items-center gap-4">
                        <Button disabled={processing}>Update lease</Button>
                    </div>
                </form>
            </div>
        </>
